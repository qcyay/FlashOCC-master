...
class PrepareImageInputs(object):
    ...
    def sample_augmentation(self, H, W, flip=None, scale=None):
        ...
        if self.is_train:
            ...
            newW, newH = resize_dims # resize后的size
            # 裁剪
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam_name in cam_names:
            # 键值dict_keys(['data_path', 'type', 'sample_data_token', 'sensor2ego_translation', 'sensor2ego_rotation', 'ego2global_translation', 'ego2global_rotation', 'timestamp', 'sensor2lidar_rotation', 'sensor2lidar_translation', 'cam_intrinsic'])
            ...
            # resize，调整大小的比例，resize_dims，调整大小后的宽和高，crop，裁剪调整大小后图像的位置，flip，是否翻转，rotate，旋转角度
            resize, resize_dims, crop, flip, rotate = img_augs

            # img: PIL.Image;  post_rot: Tensor (2, 2);  post_tran: Tensor (2, )
            #img，调整大小后的图像，post_rot2，调整图像大小对应的旋转矩阵，post_tran2，调整图像大小对应的平移矩阵
            ...
            # 各相机内参矩阵，列表，包含N个tensor，尺寸为[3，3]
            intrins.append(intrin)      # 相机内参 (3, 3)
            # 各相机到自车坐标系的变换矩阵，列表，包含N个tensor，尺寸为[4，4]
            sensor2egos.append(sensor2ego)      # camera2ego变换 (4, 4)
            # 自车到全局坐标系的变换矩阵，列表，包含N个tensor，尺寸为[4，4]
...
class LoadAnnotationsBEVDepth(object):
    ...
    def bev_transform(self, gt_boxes, rotate_angle, scale_ratio, flip_dx,
        ...
        if flip_dy:     # 沿着x轴翻转
            flip_mat = flip_mat @ torch.Tensor([[1, 0, 0], [0, -1, 0],
                                                [0, 0, 1]])
        # TODO 搞清楚这里变换矩阵是怎么得到的，又是怎么作用于坐标的
